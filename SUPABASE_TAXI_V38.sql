...
BEGIN
  ...
  UPDATE public.service_requests SET status='accepted',updated_at=now() WHERE id=p_request_id RETURNING id INTO v_id;
...
BEGIN
  ...
  UPDATE public.service_requests SET status='cancelled',updated_at=now() WHERE id=p_request_id RETURNING id INTO v_id;
